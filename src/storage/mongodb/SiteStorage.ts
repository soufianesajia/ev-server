...
export default class SiteStorage {
  ...
  public static async getSiteImage(tenant: Tenant, id: string): Promise<Image> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getSiteImage');
    // Check Tenant
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getSiteImage', uniqueTimerID, siteImageMDB);
    return {
      id: id,
  ...
  public static async removeUsersFromSite(tenant: Tenant, siteID: string, userIDs: string[]): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'removeUsersFromSite');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Site provided?
    if (siteID) {
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'removeUsersFromSite', uniqueTimerID, userIDs);
  }

  public static async addUsersToSite(tenant: Tenant, siteID: string, userIDs: string[]): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'addUsersToSite');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Site provided?
    if (siteID) {
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'addUsersToSite', uniqueTimerID, userIDs);
  }
  ...
  public static async getSiteUsers(tenant: Tenant,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getSitesUsers');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    if (dbParams.onlyRecordCount) {
      await Logging.traceEnd(tenant, MODULE_NAME, 'getSitesUsers', uniqueTimerID, usersCountMDB);
      return {
    ...
    const siteUsersMDB = await global.database.getCollection<{ user: User; siteID: string; siteAdmin: boolean; siteOwner: boolean }>(tenant.id, 'siteusers')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getSitesUsers', uniqueTimerID, siteUsersMDB);
    return {
  ...
  public static async updateSiteOwner(tenant: Tenant, siteID: string, userID: string, siteOwner: boolean): Promise<void> {
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'updateSiteOwner');
    DatabaseUtils.checkTenantObject(tenant);
    ...
    await Logging.traceEnd(tenant, MODULE_NAME, 'updateSiteOwner', uniqueTimerID, { siteID, userID });
  }

  public static async updateSiteUserAdmin(tenant: Tenant, siteID: string, userID: string, siteAdmin: boolean): Promise<void> {
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'updateSiteUserAdmin');
    DatabaseUtils.checkTenantObject(tenant);
    ...
    await Logging.traceEnd(tenant, MODULE_NAME, 'updateSiteUserAdmin', uniqueTimerID, { siteID, userID, siteAdmin });
  }

  public static async saveSite(tenant: Tenant, siteToSave: Site, saveImage = true): Promise<string> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveSite');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveSite', uniqueTimerID, siteMDB);
  ...
  public static async saveSiteImage(tenant: Tenant, siteID: string, siteImageToSave: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveSiteImage');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Modify
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveSiteImage', uniqueTimerID, siteImageToSave);
  }
  ...
  public static async getSites(tenant: Tenant,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getSites');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Clone before updating the values
    dbParams = Utils.cloneObject(dbParams);
    ...
    // Check if only the total count is requested
    if (dbParams.onlyRecordCount) {
      await Logging.traceEnd(tenant, MODULE_NAME, 'getSites', uniqueTimerID, sitesCountMDB);
      return {
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getSites', uniqueTimerID, sites);
    return {
  ...
  public static async deleteSites(tenant: Tenant, ids: string[]): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteSites');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteSites', uniqueTimerID, { ids });
  }

  public static async deleteCompanySites(tenant: Tenant, companyID: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteCompanySites');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteCompanySites', uniqueTimerID, { companyID });
  }

  public static async siteHasUser(tenant: Tenant, siteID: string, userID: string): Promise<boolean> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'siteHasUser');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Exec
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteCompanySites', uniqueTimerID, { siteID });
    // Check
    if (!result) {
      return false;
    }
