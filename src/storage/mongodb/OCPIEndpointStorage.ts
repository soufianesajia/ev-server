...
export default class OCPIEndpointStorage {
  ...
  static async saveOcpiEndpoint(tenant: Tenant, ocpiEndpointToSave: OCPIEndpoint): Promise<string> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveOcpiEndpoint');
    // Check Tenant
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveOcpiEndpoint', uniqueTimerID, ocpiEndpointMDB);
    // Create
  ...
  static async getOcpiEndpoints(tenant: Tenant,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getOcpiEndpoints');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    if (dbParams.onlyRecordCount) {
      await Logging.traceEnd(tenant, MODULE_NAME, 'getOcpiEndpoints', uniqueTimerID, ocpiEndpointsCountMDB);
      return {
    ...
    const ocpiEndpointsMDB = await global.database.getCollection<OCPIEndpoint>(tenant.id, 'ocpiendpoints')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getOcpiEndpoints', uniqueTimerID, ocpiEndpointsMDB);
    // Ok
    return {
      count: (ocpiEndpointsCountMDB.length > 0 ? ocpiEndpointsCountMDB[0].count : 0),
  ...
  static async deleteOcpiEndpoint(tenant: Tenant, id: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteOcpiEndpoint');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Delete OcpiEndpoint
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteOcpiEndpoint', uniqueTimerID, { id });
  }

  static async deleteOcpiEndpoints(tenant: Tenant): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteOcpiEndpoints');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Delete OcpiEndpoint
    await global.database.getCollection<OCPIEndpoint>(tenant.id, 'ocpiendpoints').deleteMany({});
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteOcpiEndpoints', uniqueTimerID);
  }
}
