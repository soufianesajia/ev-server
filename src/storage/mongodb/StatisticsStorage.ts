...
export default class StatisticsStorage {
  ...
  static async getChargingStationStats(tenant: Tenant, params: StatisticFilter, groupBy: string): Promise<ChargingStationStats[]> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getChargingStationStats');
    // Check Tenant
    ...
    const chargingStationStatsMDB = await global.database.getCollection<any>(tenant.id, 'transactions')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getChargingStationStats', uniqueTimerID, chargingStationStatsMDB);
  ...
  static async getUserStats(tenant: Tenant, params: StatisticFilter, groupBy: string): Promise<UserStats[]> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getUserStats');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Build filter
    const filters: any = {};
    ...
    const userStatsMDB = await global.database.getCollection<any>(tenant.id, 'transactions')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getUserStats', uniqueTimerID, userStatsMDB);
    return userStatsMDB;
  }
}
