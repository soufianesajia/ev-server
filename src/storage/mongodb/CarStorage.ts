...
export default class CarStorage {
  ...
  public static async getCarCatalogs(
    ...
    if (dbParams.onlyRecordCount) {
      ...
      await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'getCarCatalogs', uniqueTimerID, carCatalogsCountMDB);
      return {
    ...
    const carCatalogs = await global.database.getCollection<any>(Constants.DEFAULT_TENANT, 'carcatalogs')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'getCarCatalogs', uniqueTimerID, carCatalogs);
    // Ok
    return {
  ...
  public static async saveCarCatalog(carToSave: CarCatalog): Promise<number> {
    ...
    // Debug
    await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'saveCarCatalog', uniqueTimerID, carMDB);
    return carToSave.id;
  }
  ...
  public static async saveCarImage(carID: number, carImageToSave: string): Promise<void> {
    ...
    // Debug
    await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'saveCarImage', uniqueTimerID, carImageToSave);
  }
  ...
  public static async deleteCarImages(carID: number): Promise<void> {
    ...
    // Debug
    await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'deleteCarImages', uniqueTimerID, carID);
  }
  ...
  public static async getCarCatalogImage(id: number): Promise<{ id: number; image: string }> {
    ...
    const carCatalogImageMDB = await global.database.getCollection<{ _id: number; image: string }>(Constants.DEFAULT_TENANT, 'carcatalogs')
      .findOne({ _id: id });
    // Debug
    await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'getCarCatalogImage', uniqueTimerID, carCatalogImageMDB);
    return {
      id: id,
  ...
  public static async getCarCatalogImages(id: number = Constants.UNKNOWN_NUMBER_ID, dbParams?: DbParams): Promise<DataResult<Image>> {
    ...
    // Check if only the total count is requested
    if (dbParams.onlyRecordCount) {
      await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'getCarCatalogImages', uniqueTimerID, carCatalogImagesCountMDB);
      return {
    ...
    const carCatalogImages = await global.database.getCollection<any>(Constants.DEFAULT_TENANT, 'carcatalogimages')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'getCarCatalogImages', uniqueTimerID, carCatalogImages);
    return {
  ...
  public static async getCarMakers(params: { search?: string } = {}, projectFields?: string[]): Promise<DataResult<CarMaker>> {
    ...
    const carMakersMDB = await global.database.getCollection<any>(Constants.DEFAULT_TENANT, 'carcatalogs')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'getCarMakers', uniqueTimerID, carMakersMDB);
    return {
  ...
  public static async saveCar(tenant: Tenant, carToSave: Car): Promise<string> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveCar');
    // Check Tenant
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveCar', uniqueTimerID, carMDB);
  ...
  public static async getCars(tenant: Tenant,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getCars');
    // Clone before updating the values
    dbParams = Utils.cloneObject(dbParams);
    // Check Limit
    dbParams.limit = Utils.checkRecordLimit(dbParams.limit);
    ...
    if (dbParams.onlyRecordCount) {
      // Return only the count
      await Logging.traceEnd(tenant, MODULE_NAME, 'getCars', uniqueTimerID, carsCountMDB);
      return {
    ...
    const cars = await global.database.getCollection<Car>(tenant.id, 'cars')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getCars', uniqueTimerID, cars);
    return {
  ...
  public static async clearDefaultUserCar(tenant: Tenant, userID: string): Promise<void> {
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'clearDefaultUserCar');
    DatabaseUtils.checkTenantObject(tenant);
    ...
    await Logging.traceEnd(tenant, MODULE_NAME, 'clearDefaultUserCar', uniqueTimerID, { userID });
  }

  public static async deleteCar(tenant: Tenant, carID: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteCar');
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteCar', uniqueTimerID, { carID });
  }
}
