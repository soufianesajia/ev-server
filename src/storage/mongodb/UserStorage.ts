...
export default class UserStorage {
  public static async getEndUserLicenseAgreement(tenant: Tenant, language = 'en'): Promise<Eula> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getEndUserLicenseAgreement');
    // Check Tenant
    ...
    if (!Utils.isEmptyArray(eulasMDB)) {
      ...
      if (currentEulaHash !== eulaMDB.hash) {
        ...
        await global.database.getCollection<Eula>(tenant.id, 'eulas')
          .insertOne(eula);
        // Debug
        await Logging.traceEnd(tenant, MODULE_NAME, 'getEndUserLicenseAgreement', uniqueTimerID, eula);
        return eula;
      }
      // Debug
      await Logging.traceEnd(tenant, MODULE_NAME, 'getEndUserLicenseAgreement', uniqueTimerID, eulaMDB);
      return eulaMDB;
    }
    // Create default
    const eula = {
  ...
  public static async getUserImage(tenant: Tenant, id: string): Promise<Image> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getUserImage');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Read DB
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getUserImage', uniqueTimerID, userImageMDB);
    return {
  ...
  public static async removeSitesFromUser(tenant: Tenant, userID: string, siteIDs: string[]): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'removeSitesFromUser');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // User provided?
    if (userID) {
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'removeSitesFromUser', uniqueTimerID, siteIDs);
  }

  public static async addSitesToUser(tenant: Tenant, userID: string, siteIDs: string[]): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'addSitesToUser');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // At least one Site
    if (!Utils.isEmptyArray(siteIDs)) {
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'addSitesToUser', uniqueTimerID, siteIDs);
  }
  ...
  public static async addSiteToUser(tenant: Tenant, userID: string, siteID: string): Promise<string> {
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveUser', uniqueTimerID, siteID);
    return siteUserMDB._id;
  }

  public static async saveUser(tenant: Tenant, userToSave: User, saveImage = false): Promise<string> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveUser');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveUser', uniqueTimerID, userMDB);
  ...
  public static async saveImportedUser(tenant: Tenant, importedUserToSave: ImportedUser): Promise<string> {
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveImportedUser');
    const userMDB = {
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveImportedUser', uniqueTimerID, userMDB);
    return userMDB._id.toString();
  }

  public static async saveImportedUsers(tenant: Tenant, importedUsersToSave: ImportedUser[]): Promise<number> {
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveImportedUsers');
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveImportedUsers', uniqueTimerID, importedUsersToSave);
    return result.insertedCount;
  }

  public static async deleteImportedUser(tenant: Tenant, importedUserID: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteImportedUser');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Delete
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteImportedUser', uniqueTimerID, { id: importedUserID });
  }

  public static async deleteImportedUsers(tenant: Tenant): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteImportedUsers');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Delete
    await global.database.getCollection<any>(tenant.id, 'importedusers').deleteMany({});
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteImportedUsers', uniqueTimerID);
  }
  ...
  public static async saveUserPassword(tenant: Tenant, userID: string,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveUserPassword');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Modify and return the modified document
    await global.database.getCollection<any>(tenant.id, 'users').findOneAndUpdate(
      ...
      { $set: params });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveUserPassword', uniqueTimerID);
  }

  public static async saveUserStatus(tenant: Tenant, userID: string, status: UserStatus): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveUserStatus');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Modify and return the modified document
    await global.database.getCollection<any>(tenant.id, 'users').findOneAndUpdate(
      ...
      { $set: { status } });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveUserStatus', uniqueTimerID);
  }
  ...
  public static async saveUserMobileToken(tenant: Tenant, userID: string,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveUserMobileToken');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Modify and return the modified document
    await global.database.getCollection<any>(tenant.id, 'users').findOneAndUpdate(
      ...
      { $set: params });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveUserMobileToken', uniqueTimerID);
  }

  public static async saveUserMobilePhone(tenant: Tenant, userID: string,
      params: { mobile?: string; phone?: string; }): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveUserMobilePhone');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Modify and return the modified document
    await global.database.getCollection<any>(tenant.id, 'users').findOneAndUpdate(
      ...
      { $set: params });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveUserMobilePhone', uniqueTimerID);
  }

  public static async saveUserRole(tenant: Tenant, userID: string, role: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveUserRole');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Modify and return the modified document
    await global.database.getCollection<any>(tenant.id, 'users').findOneAndUpdate(
      ...
      { $set: { role } });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveUserRole', uniqueTimerID);
  }
  ...
  public static async saveUserEULA(tenant: Tenant, userID: string,
    ...
    await global.database.getCollection<any>(tenant.id, 'users').findOneAndUpdate(
      ...
      { $set: params });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveUserRole', uniqueTimerID, params);
  }
  ...
  public static async saveUserAccountVerification(tenant: Tenant, userID: string,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveUserAccountVerification');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Modify and return the modified document
    await global.database.getCollection<any>(tenant.id, 'users').findOneAndUpdate(
      ...
      { $set: params });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveUserAccountVerification', uniqueTimerID, params);
  }
  ...
  public static async saveUserAdminData(tenant: Tenant, userID: string,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveUserAdminData');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Set data
    ...
    await global.database.getCollection<any>(tenant.id, 'users').findOneAndUpdate(
      ...
      { $set: updatedUserMDB });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveUserAdminData', uniqueTimerID, params);
  }

  public static async saveUserBillingData(tenant: Tenant, userID: string, billingData: BillingUserData): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveUserBillingData');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    if (billingData) {
    // Set data
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveUserBillingData', uniqueTimerID, billingData);
  }

  public static async saveUserImage(tenant: Tenant, userID: string, userImageToSave: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveUserImage');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    await global.database.getCollection<any>(tenant.id, 'userimages').findOneAndUpdate(
      ...
      { upsert: true, returnDocument: 'after' });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveUserImage', uniqueTimerID, userImageToSave);
  }
  ...
  public static async getUsers(tenant: Tenant,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getUsers');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    if (dbParams.onlyRecordCount) {
      ...
      await Logging.traceEnd(tenant, MODULE_NAME, 'getUsers', uniqueTimerID, usersCountMDB);
      return {
    ...
    const usersMDB = await global.database.getCollection<User>(tenant.id, 'users')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getUsers', uniqueTimerID, usersMDB);
    // Ok
    return {
  ...
  public static async getImportedUsersCount(tenant: Tenant): Promise<number> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getImportedUsersCount');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Count documents
    const nbrOfDocuments = await global.database.getCollection<any>(tenant.id, 'importedusers').countDocuments();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getImportedUsersCount', uniqueTimerID);
  ...
  public static async getImportedUsers(tenant: Tenant,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getImportedUsers');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Clone before updating the values
    dbParams = Utils.cloneObject(dbParams);
    ...
    if (dbParams.onlyRecordCount) {
      // Return only the count
      await Logging.traceEnd(tenant, MODULE_NAME, 'getImportedUsers', uniqueTimerID, usersImportCountMDB);
      return {
    ...
    const usersImportMDB = await global.database.getCollection<any>(tenant.id, 'importedusers')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getUsersImport', uniqueTimerID, usersImportMDB);
    // Ok
    return {
  ...
  public static async deleteUser(tenant: Tenant, id: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteUser');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Delete User Image
    ...
    await global.database.getCollection<any>(tenant.id, 'users')
      .findOneAndDelete({ '_id': DatabaseUtils.convertToObjectID(id) });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteUser', uniqueTimerID, { id });
  }
  ...
  public static async getUserSites(tenant: Tenant,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getUserSites');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Clone before updating the values
    dbParams = Utils.cloneObject(dbParams);
    ...
    // Check if only the total count is requested
    if (dbParams.onlyRecordCount) {
      await Logging.traceEnd(tenant, MODULE_NAME, 'getUserSites', uniqueTimerID, sitesCountMDB);
      return {
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getUserSites', uniqueTimerID, siteUsersMDB);
    // Ok
    return {
  ...
  private static getEndUserLicenseAgreementFromFile(language = 'en'): string {
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'getEndUserLicenseAgreementFromFile');
    let eulaText = null;
    try {
    ...
    // Debug
    void Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'getEndUserLicenseAgreementFromFile', uniqueTimerID, eulaText);
    return eulaText;
  }
}
