...
export default class MigrationStorage {
  static async getMigrations(): Promise<Migration[]> {
    // Debug
    const uniqueTimerID = Logging.traceStart(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'getMigrations');
    const aggregation = [];
    // Handle the ID
    ...
    const migrationsMDB = await global.database.getCollection<Migration>(Constants.DEFAULT_TENANT, 'migrations')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'getMigrations', uniqueTimerID, migrationsMDB);
    return migrationsMDB;
  }

  static async saveMigration(migrationToSave: Migration): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'saveMigration');
    // Transfer
    const migrationMDB = {
    ...
    // Debug
    await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'saveMigration', uniqueTimerID, migrationMDB);
  }
}
