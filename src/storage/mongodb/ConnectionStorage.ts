...
export default class ConnectionStorage {
  ...
  static async saveConnection(tenant: Tenant, connectionToSave: Connection): Promise<string> {
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveConnection');
    ...
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveConnection', uniqueTimerID, connectionMDB);
  ...
  static async getConnectionByConnectorIdAndUserId(tenant: Tenant, connectorId: string, userId: string, projectFields?: string[]): Promise<Connection> {
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getConnectionByConnectorIdAndUserId');
    DatabaseUtils.checkTenantObject(tenant);
    const aggregation = [];
    aggregation.push({
    ...
    await Logging.traceEnd(tenant, MODULE_NAME, 'getConnectionByConnectorIdAndUserId', uniqueTimerID, connections);
    return connection;
  }

  static async getConnectionsByUserId(tenant: Tenant, userID: string, projectFields?: string[]): Promise<DataResult<Connection>> {
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getConnectionsByUserId');
    DatabaseUtils.checkTenantObject(tenant);
    const aggregation = [];
    aggregation.push({
    ...
    const connectionsMDB = await global.database.getCollection<Connection>(tenant.id, 'connections')
      ...
      .toArray();
    await Logging.traceEnd(tenant, MODULE_NAME, 'getConnectionByUserId', uniqueTimerID, connectionsMDB);
    return {
  ...
  static async getConnection(tenant: Tenant, id: string = Constants.UNKNOWN_OBJECT_ID, projectFields?: string[]): Promise<Connection> {
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getConnection');
    DatabaseUtils.checkTenantObject(tenant);
    const aggregation = [];
    // Filters
    aggregation.push({
    ...
    await Logging.traceEnd(tenant, MODULE_NAME, 'getConnection', uniqueTimerID, connections);
    return connection;
  }

  static async deleteConnectionById(tenant: Tenant, id: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteConnection');
    // Check
    DatabaseUtils.checkTenantObject(tenant);
    // Delete
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteConnection', uniqueTimerID, { id });
  }

  static async deleteConnectionByUserId(tenant: Tenant, userID: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteConnectionByUser');
    // Check
    DatabaseUtils.checkTenantObject(tenant);
    // Delete
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteConnectionByUser', uniqueTimerID, { userID });
  }
}
