...
export default class OICPEndpointStorage {
  ...
  static async saveOicpEndpoint(tenant: Tenant, oicpEndpointToSave: OICPEndpoint): Promise<string> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveOicpEndpoint');
    // Check Tenant
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveOicpEndpoint', uniqueTimerID, { oicpEndpointToSave: oicpEndpointToSave });
    // Create
  ...
  static async getOicpEndpoints(tenant: Tenant,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getOicpEndpoints');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    if (dbParams.onlyRecordCount) {
      await Logging.traceEnd(tenant, MODULE_NAME, 'getOicpEndpoints', uniqueTimerID, oicpEndpointsCountMDB);
      return {
    ...
    const oicpEndpointsMDB = await global.database.getCollection<any>(tenant.id, 'oicpendpoints')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getOicpEndpoints', uniqueTimerID, oicpEndpointsMDB);
    // Ok
    return {
      count: (oicpEndpointsCountMDB.length > 0 ? oicpEndpointsCountMDB[0].count : 0),
  ...
  static async deleteOicpEndpoint(tenant: Tenant, id: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteOicpEndpoint');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Delete OicpEndpoint
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteOicpEndpoint', uniqueTimerID, { id });
  }

  static async deleteOicpEndpoints(tenant: Tenant): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteOicpEndpoints');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Delete OicpEndpoint
    await global.database.getCollection<any>(tenant.id, 'oicpendpoints').deleteMany({});
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteOicpEndpoints', uniqueTimerID);
  }
}
