...
export default class NotificationStorage {
  static async getNotifications(tenant: Tenant,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getNotifications');
    // Check Tenant
    ...
    const notificationsMDB = await global.database.getCollection<any>(tenant.id, 'notifications')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getNotifications', uniqueTimerID, notificationsMDB);
    // Ok
    return {
      count: (notificationsCountMDB.length > 0 ? notificationsCountMDB[0].count : 0),
  ...
  static async saveNotification(tenant: Tenant, notificationToSave: Partial<Notification>): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveNotification');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveNotification', uniqueTimerID, ocpiEndpointMDB);
  }
}
