...
export default class AsyncTaskStorage {
  ...
  public static async saveAsyncTask(asyncTaskToSave: AsyncTask): Promise<string> {
    // Debug
    const uniqueTimerID = Logging.traceStart(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'saveAsyncTask');
    // Set
    ...
    // Debug
    await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'saveAsyncTask', uniqueTimerID, asyncTaskMDB);
  ...
  public static async getAsyncTasks(params: { status?: AsyncTaskStatus, asyncTaskIDs?: string[] } = {},
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'getAsyncTasks');
    ...
    if (dbParams.onlyRecordCount) {
      ...
      await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'getAsyncTasks', uniqueTimerID, asyncTasksCountMDB);
      return {
    ...
    const asyncTasksMDB = await global.database.getCollection<any>(Constants.DEFAULT_TENANT, 'asynctasks')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'getAsyncTasks', uniqueTimerID, asyncTasksMDB);
    return {
  ...
  public static async updateRunningAsyncTaskToPending(): Promise<number> {
    // Debug
    const uniqueTimerID = Logging.traceStart(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'updateRunningAsyncTaskToPending');
    ...
    // Debug
    await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'updateRunningAsyncTaskToPending', uniqueTimerID);
    return result.modifiedCount;
  }

  public static async deleteAsyncTask(id: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'deleteAsyncTask');
    // Delete the AsyncTask
    await global.database.getCollection<AsyncTask>(Constants.DEFAULT_TENANT, 'asynctasks')
      .findOneAndDelete({ '_id': id });
    // Debug
    await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'deleteAsyncTask', uniqueTimerID, { id });
  }
}
