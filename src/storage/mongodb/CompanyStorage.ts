...
export default class CompanyStorage {
  ...
  public static async getCompanyLogo(tenant: Tenant, id: string): Promise<{ id: string; logo: string }> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getCompanyLogo');
    // Check Tenant
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getCompanyLogo', uniqueTimerID, companyLogoMDB);
    return {
      id: id,
  ...
  public static async saveCompany(tenant: Tenant, companyToSave: Company, saveLogo = true): Promise<string> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveCompany');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Set
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveCompany', uniqueTimerID, companyMDB);
  ...
  public static async getCompanies(tenant: Tenant,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getCompanies');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    if (dbParams.onlyRecordCount) {
      ...
      await Logging.traceEnd(tenant, MODULE_NAME, 'getCompanies', uniqueTimerID, companiesCountMDB);
      return {
    ...
    const companiesMDB = await global.database.getCollection<any>(tenant.id, 'companies')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getCompanies', uniqueTimerID, companiesMDB);
    // Ok
    return {
  ...
  public static async deleteCompany(tenant: Tenant, id: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteCompany');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    await global.database.getCollection<any>(tenant.id, 'companylogos')
      .findOneAndDelete({ '_id': DatabaseUtils.convertToObjectID(id) });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteCompany', uniqueTimerID, { id });
  }

  private static async saveCompanyLogo(tenant: Tenant, companyID: string, companyLogoToSave: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveCompanyLogo');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Modify
    await global.database.getCollection<any>(tenant.id, 'companylogos').findOneAndUpdate(
      ...
      { upsert: true });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveCompanyLogo', uniqueTimerID, companyLogoToSave);
  }
}
