...
export default class OCPPStorage {
  public static async saveAuthorize(tenant: Tenant, authorize: OCPPAuthorizeRequestExtended): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveAuthorize');
    // Check Tenant
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveAuthorize', uniqueTimerID, authorize);
  }
  ...
  public static async getAuthorizes(tenant: Tenant, params: {dateFrom?: Date; chargeBoxID?: string; tagID?: string},
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getAuthorizes');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    const authorizesMDB = await global.database.getCollection<any>(tenant.id, 'authorizes')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getAuthorizes', uniqueTimerID, authorizesMDB);
    // Ok
    return {
  ...
  public static async saveHeartbeat(tenant: Tenant, heartbeat: OCPPHeartbeatRequestExtended): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveHeartbeat');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveHeartbeat', uniqueTimerID, heartbeat);
  }
  ...
  public static async getStatusNotifications(tenant: Tenant,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getStatusNotifications');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Clone before updating the values
    dbParams = Utils.cloneObject(dbParams);
    ...
    const statusNotificationsMDB = await global.database.getCollection<any>(tenant.id, 'statusnotifications')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getStatusNotifications', uniqueTimerID, statusNotificationsMDB);
    // Ok
    return {
  ...
  public static async getLastStatusNotifications(tenant: Tenant,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getLastStatusNotifications');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Set the filters
    const filters: FilterParams = {};
    ...
    const statusNotificationsMDB = await global.database.getCollection<any>(tenant.id, 'statusnotifications')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getLastStatusNotifications', uniqueTimerID, statusNotificationsMDB);
    // Ok
  ...
  public static async saveStatusNotification(tenant: Tenant, statusNotificationToSave: OCPPStatusNotificationRequestExtended): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveStatusNotification');
    // Set
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveStatusNotification', uniqueTimerID, statusNotificationMDB);
  }

  public static async saveDataTransfer(tenant: Tenant, dataTransfer: OCPPDataTransferRequestExtended): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveDataTransfer');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Set the ID
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveDataTransfer', uniqueTimerID, dataTransfer);
  }

  public static async saveBootNotification(tenant: Tenant, bootNotification: OCPPBootNotificationRequestExtended): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveBootNotification');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Insert
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveBootNotification', uniqueTimerID, bootNotification);
  }
  ...
  public static async getBootNotifications(tenant: Tenant, params: {chargeBoxID?: string},
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getBootNotifications');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Clone before updating the values
    dbParams = Utils.cloneObject(dbParams);
    ...
    const bootNotificationsMDB = await global.database.getCollection<any>(tenant.id, 'bootnotifications')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getBootNotifications', uniqueTimerID, bootNotificationsMDB);
    // Ok
    return {
  ...
  public static async saveDiagnosticsStatusNotification(tenant: Tenant, diagnosticsStatusNotification: OCPPDiagnosticsStatusNotificationRequestExtended): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveDiagnosticsStatusNotification');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveDiagnosticsStatusNotification', uniqueTimerID, diagnosticsStatusNotification);
  }
  ...
  public static async saveFirmwareStatusNotification(tenant: Tenant, firmwareStatusNotification: OCPPFirmwareStatusNotificationRequestExtended): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveFirmwareStatusNotification');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Set the ID
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveFirmwareStatusNotification', uniqueTimerID, firmwareStatusNotification);
  }

  public static async saveMeterValues(tenant: Tenant, meterValuesToSave: OCPPNormalizedMeterValues): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveMeterValues');
    // Check
    DatabaseUtils.checkTenantObject(tenant);
    // Save all
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveMeterValues', uniqueTimerID, meterValuesToSave);
  }
  ...
  public static async getMeterValues(tenant: Tenant, params: { transactionId: number },
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getMeterValues');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Clone before updating the values
    dbParams = Utils.cloneObject(dbParams);
    ...
    const meterValuesMDB = await global.database.getCollection<any>(tenant.id, 'metervalues')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getMeterValues', uniqueTimerID, meterValuesMDB);
    // Ok
    return {
