...
export default class ChargingStationStorage {
  ...
  public static async updateChargingStationTemplatesFromFile(): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'updateChargingStationTemplatesFromFile');
    // Read File
    ...
    try {
      ...
      await Logging.logActionExceptionMessage(Constants.DEFAULT_TENANT_OBJECT, ServerAction.UPDATE_CHARGING_STATION_TEMPLATES, error);
      return;
    }
    ...
    // Debug
    await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'updateChargingStationTemplatesFromFile', uniqueTimerID, chargingStationTemplates);
  }

  public static async getChargingStationTemplates(chargePointVendor?: string): Promise<ChargingStationTemplate[]> {
    // Debug
    const uniqueTimerID = Logging.traceStart(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'getChargingStationTemplates');
    // Create Aggregation
    const aggregation = [];
    // Change ID
    ...
    // Debug
    await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'getChargingStationTemplates', uniqueTimerID, chargingStationTemplatesMDB);
    return chargingStationTemplates;
  }

  public static async deleteChargingStationTemplates(): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'deleteChargingStationTemplates');
    // Delete all records
    await global.database.getCollection<ChargingStationTemplate>(Constants.DEFAULT_TENANT, 'chargingstationtemplates').deleteMany(
      { qa: { $not: { $eq: true } } }
    );
    // Debug
    await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'deleteChargingStationTemplates', uniqueTimerID);
  }

  public static async saveChargingStationTemplate(chargingStationTemplate: ChargingStationTemplate): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'saveChargingStationTemplate');
    ...
    await global.database.getCollection<ChargingStationTemplate>(Constants.DEFAULT_TENANT, 'chargingstationtemplates').findOneAndReplace(
      ...
      { upsert: true });
    // Debug
    await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'saveChargingStationTemplate', uniqueTimerID, chargingStationTemplate);
  }
  ...
  public static async getChargingStations(tenant: Tenant,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getChargingStations');
    // Check Tenant
    ...
    if (dbParams.onlyRecordCount) {
      ...
      await Logging.traceEnd(tenant, MODULE_NAME, 'getChargingStations', uniqueTimerID, chargingStationsCountMDB);
      return {
    ...
    const chargingStationsMDB = await global.database.getCollection<ChargingStation>(tenant.id, 'chargingstations')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getChargingStations', uniqueTimerID, chargingStationsMDB);
    return {
  ...
  public static async saveChargingStation(tenant: Tenant, chargingStationToSave: ChargingStation): Promise<string> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveChargingStation');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Build Request
    ...
    await global.database.getCollection<any>(tenant.id, 'chargingstations').findOneAndUpdate(
      ...
      { upsert: true });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveChargingStation', uniqueTimerID, chargingStationMDB);
  ...
  public static async saveChargingStationConnectors(tenant: Tenant, id: string, connectors: Connector[], backupConnectors?: Connector[]): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveChargingStationConnectors');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    await global.database.getCollection<any>(tenant.id, 'chargingstations').findOneAndUpdate(
      ...
      { upsert: true });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveChargingStationConnectors', uniqueTimerID, connectors);
  }
  ...
  public static async saveChargingStationCFApplicationIDAndInstanceIndex(tenant: Tenant, id: string,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveChargingStationCFApplicationIDAndInstanceIndex');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Modify document
    await global.database.getCollection<ChargingStation>(tenant.id, 'chargingstations').findOneAndUpdate(
      ...
      { upsert: true });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveChargingStationCFApplicationIDAndInstanceIndex', uniqueTimerID, cfApplicationIDAndInstanceIndex);
  }

  public static async saveChargingStationOicpData(tenant: Tenant, id: string,
      oicpData: ChargingStationOicpData): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveChargingStationOicpData');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Modify document
    await global.database.getCollection<ChargingStation>(tenant.id, 'chargingstations').findOneAndUpdate(
      ...
      { upsert: false });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveChargingStationOicpData', uniqueTimerID, oicpData);
  }
  ...
  public static async saveChargingStationLastSeen(tenant: Tenant, id: string,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveChargingStationLastSeen');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Set data
    // Modify document
    await global.database.getCollection<ChargingStation>(tenant.id, 'chargingstations').findOneAndUpdate(
      ...
      { upsert: true });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveChargingStationLastSeen', uniqueTimerID, params);
  }

  public static async saveChargingStationOcpiData(tenant: Tenant, id: string,
      ocpiData: ChargingStationOcpiData): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveChargingStationOcpiData');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Modify document
    await global.database.getCollection<ChargingStation>(tenant.id, 'chargingstations').findOneAndUpdate(
      ...
      { upsert: false });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveChargingStationOcpiData', uniqueTimerID, ocpiData);
  }
  ...
  public static async saveChargingStationRemoteAuthorizations(tenant: Tenant, id: string,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveChargingStationRemoteAuthorizations');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Modify document
    await global.database.getCollection<ChargingStation>(tenant.id, 'chargingstations').findOneAndUpdate(
      ...
      { upsert: false });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveChargingStationRemoteAuthorizations', uniqueTimerID, remoteAuthorizations);
  }
  ...
  public static async saveChargingStationFirmwareStatus(tenant: Tenant, id: string, firmwareUpdateStatus: OCPPFirmwareStatus): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveChargingStationFirmwareStatus');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Modify document
    await global.database.getCollection<ChargingStation>(tenant.id, 'chargingstations').findOneAndUpdate(
      ...
      { upsert: true });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveChargingStationFirmwareStatus', uniqueTimerID, firmwareUpdateStatus);
  }

  public static async deleteChargingStation(tenant: Tenant, id: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteChargingStation');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteChargingStation', uniqueTimerID, { id });
  }
  ...
  static async saveOcppParameters(tenant: Tenant, parameters: ChargingStationOcppParameters): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveOcppParameters');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Modify
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveOcppParameters', uniqueTimerID, parameters);
  }

  public static async getOcppParameters(tenant: Tenant, id: string): Promise<DataResult<OcppParameter>> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getOcppParameters');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Read DB
    ...
    if (parametersMDB) {
      ...
      // Debug
      await Logging.traceEnd(tenant, MODULE_NAME, 'getOcppParameters', uniqueTimerID, parametersMDB);
      return {
  ...
  public static async getChargingProfiles(tenant: Tenant,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getChargingProfiles');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Clone before updating the values
    dbParams = Utils.cloneObject(dbParams);
    ...
    // Check if only the total count is requested
    if (dbParams.onlyRecordCount) {
      await Logging.traceEnd(tenant, MODULE_NAME, 'getChargingProfiles', uniqueTimerID, chargingProfilesCountMDB);
      return {
    ...
    const chargingProfilesMDB = await global.database.getCollection<ChargingProfile>(tenant.id, 'chargingprofiles')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getChargingProfiles', uniqueTimerID, chargingProfilesMDB);
    return {
  ...
  public static async saveChargingProfile(tenant: Tenant, chargingProfileToSave: ChargingProfile): Promise<string> {
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveChargingProfile');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    await global.database.getCollection<any>(tenant.id, 'chargingprofiles').findOneAndUpdate(
      ...
      { upsert: true });
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveChargingProfile', uniqueTimerID, chargingProfileMDB);
    return chargingProfileFilter._id as string;
  }

  public static async deleteChargingProfile(tenant: Tenant, id: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteChargingProfile');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    await global.database.getCollection<any>(tenant.id, 'chargingprofiles')
      .findOneAndDelete({ '_id': id });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteChargingProfile', uniqueTimerID, { id });
  }
  ...
  public static async deleteChargingProfiles(tenant: Tenant, chargingStationID: string): Promise<void> {
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteChargingProfile', uniqueTimerID, { chargingStationID });
  }

  public static getChargingStationFirmware(filename: string): GridFSBucketReadStream {
    const uniqueTimerID = Logging.traceStart(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'getChargingStationFirmware');
    // Get the bucket
    ...
    void Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'getChargingStationFirmware', uniqueTimerID, firmware);
    return firmware;
  }

  public static putChargingStationFirmware(filename: string): GridFSBucketWriteStream {
    const uniqueTimerID = Logging.traceStart(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'putChargingStationFirmware');
    // Get the bucket
    const bucket: GridFSBucket = global.database.getGridFSBucket('default.firmwares');
    // Put the file
    const firmware = bucket.openUploadStream(filename);
    void Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'putChargingStationFirmware', uniqueTimerID, firmware);
    return firmware;
  }
