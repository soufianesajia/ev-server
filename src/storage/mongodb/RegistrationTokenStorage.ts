...
export default class RegistrationTokenStorage {
  static async saveRegistrationToken(tenant: Tenant, registrationToken: RegistrationToken): Promise<string> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveRegistrationToken');
    // Check Tenant
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveRegistrationToken', uniqueTimerID, registrationTokenMDB);
  ...
  static async getRegistrationTokens(tenant: Tenant,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getRegistrationTokens');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    if (dbParams.onlyRecordCount) {
      ...
      await Logging.traceEnd(tenant, MODULE_NAME, 'getRegistrationTokens', uniqueTimerID, registrationTokensCountMDB);
      return {
    ...
    const registrationTokens = await global.database.getCollection<any>(tenant.id, 'registrationtokens')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getRegistrationTokens', uniqueTimerID, registrationTokens);
    // Ok
    return {
  ...
  static async deleteRegistrationToken(tenant: Tenant, id: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteRegistrationToken');
    await global.database.getCollection<any>(tenant.id, 'registrationtokens')
      .findOneAndDelete({ '_id': DatabaseUtils.convertToObjectID(id) });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteRegistrationToken', uniqueTimerID, { id });
  }
}
