...
export default class BillingStorage {
  ...
  public static async getInvoices(tenant: Tenant,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getInvoices');
    // Check Tenant
    ...
    if (dbParams.onlyRecordCount) {
      await Logging.traceEnd(tenant, MODULE_NAME, 'getInvoices', uniqueTimerID, invoicesCountMDB);
      return {
    ...
    const invoicesMDB = await global.database.getCollection<BillingInvoice>(tenant.id, 'invoices')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getInvoices', uniqueTimerID, invoicesMDB);
    return {
  ...
  public static async saveInvoice(tenant: Tenant, invoiceToSave: BillingInvoice): Promise<string> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveInvoice');
    // Build Request
    // Properties to save
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveInvoice', uniqueTimerID, invoiceMDB);
  ...
  public static async updateInvoiceAdditionalData(tenant: Tenant, invoiceToUpdate: BillingInvoice, additionalData: BillingAdditionalData): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveInvoiceAdditionalData');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveInvoiceAdditionalData', uniqueTimerID, updatedInvoiceMDB);
  }

  public static async deleteInvoice(tenant: Tenant, id: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteInvoice');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Delete the Invoice
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteInvoice', uniqueTimerID, { id });
  }
