...
export default class TagStorage {
  ...
  public static async saveTag(tenant: Tenant, tag: Tag): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveTag');
    // Check Tenant
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveTag', uniqueTimerID, tagMDB);
  }

  public static async saveImportedTag(tenant: Tenant, importedTagToSave: ImportedTag): Promise<string> {
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveImportedTag');
    const tagMDB = {
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveImportedTag', uniqueTimerID, tagMDB);
    return tagMDB._id;
  }

  public static async saveImportedTags(tenant: Tenant, importedTagsToSave: ImportedTag[]): Promise<number> {
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveImportedTags');
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveImportedTags', uniqueTimerID, importedTagsToSave);
    return result.insertedCount;
  }

  public static async deleteImportedTag(tenant: Tenant, importedTagID: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteImportedTag');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Delete
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteImportedTag', uniqueTimerID, { id: importedTagID });
  }

  public static async deleteImportedTags(tenant: Tenant): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteImportedTags');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Delete
    await global.database.getCollection<any>(tenant.id, 'importedtags').deleteMany({});
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteImportedTags', uniqueTimerID);
  }

  public static async getImportedTagsCount(tenant: Tenant): Promise<number> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getImportedTagsCount');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Count documents
    const nbrOfDocuments = await global.database.getCollection<any>(tenant.id, 'importedtags').countDocuments();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getImportedTagsCount', uniqueTimerID);
  ...
  public static async getImportedTags(tenant: Tenant,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getImportedTags');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    if (dbParams.onlyRecordCount) {
      ...
      await Logging.traceEnd(tenant, MODULE_NAME, 'getImportedTags', uniqueTimerID, tagsImportCountMDB);
      return {
    ...
    const tagsImportMDB = await global.database.getCollection<any>(tenant.id, 'importedtags')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getTagsImport', uniqueTimerID, tagsImportMDB);
    // Ok
    return {
  ...
  public static async clearDefaultUserTag(tenant: Tenant, userID: string): Promise<void> {
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'clearDefaultUserTag');
    DatabaseUtils.checkTenantObject(tenant);
    ...
    await Logging.traceEnd(tenant, MODULE_NAME, 'clearDefaultUserTag', uniqueTimerID, { userID });
  }

  public static async deleteTag(tenant: Tenant, tagID: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteTag');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Delete
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteTag', uniqueTimerID, { id: tagID });
  }

  public static async deleteTagsByUser(tenant: Tenant, userID: string): Promise<number> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteTagsByUser');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Delete
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteTagsByUser', uniqueTimerID, { id: userID });
  ...
  public static async getTags(tenant: Tenant,
    ...
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getTags');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Clone before updating the values
    dbParams = Utils.cloneObject(dbParams);
    ...
    if (dbParams.onlyRecordCount) {
      // Return only the count
      await Logging.traceEnd(tenant, MODULE_NAME, 'getTags', uniqueTimerID, tagsCountMDB);
      return {
    ...
    const tagsMDB = await global.database.getCollection<Tag>(tenant.id, 'tags')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getTags', uniqueTimerID, tagsMDB);
    // Ok
    return {
