...
export default class LockingStorage {
  public static async getLocks(params: { lockIDs?: string[]; }, dbParams: DbParams): Promise<DataResult<Lock>> {
    // Debug
    const uniqueTimerID = Logging.traceStart(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'getLocks');
    ...
    if (dbParams.onlyRecordCount) {
      ...
      await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'getLocks', uniqueTimerID, locksCountMDB);
      return {
    ...
    const locksMDB = await global.database.getCollection<Lock>(Constants.DEFAULT_TENANT, 'locks')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'getLocks', uniqueTimerID, locksMDB);
    // Ok
    return {
  ...
  public static async insertLock(lockToSave: Lock): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'insertLock');
    // Transfer
    const lockMDB = {
      _id: lockToSave.id,
      tenantID: lockToSave.tenant.id !== Constants.DEFAULT_TENANT ? DatabaseUtils.convertToObjectID(lockToSave.tenant.id) : null,
    ...
    await global.database.getCollection<any>(Constants.DEFAULT_TENANT, 'locks')
      .insertOne(lockMDB);
    // Debug
    await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'insertLock', uniqueTimerID, lockToSave);
  }

  public static async deleteLock(lockToDelete: Lock): Promise<boolean> {
    // Debug
    const uniqueTimerID = Logging.traceStart(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'deleteLock');
    // Delete
    const result = await global.database.getCollection<any>(Constants.DEFAULT_TENANT, 'locks')
      .findOneAndDelete({ '_id': lockToDelete.id });
    // Debug
    await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'deleteLock', uniqueTimerID, result);
    return result.value !== null;
  }

  public static async deleteLockByHostname(hostname:string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'deleteLockByHostname');
    // Delete
    const result = await global.database.getCollection<any>(Constants.DEFAULT_TENANT, 'locks')
      .deleteMany({ 'hostname': hostname });
    // Debug
    await Logging.traceEnd(Constants.DEFAULT_TENANT_OBJECT, MODULE_NAME, 'deleteLockByHostname', uniqueTimerID, result.deletedCount);
  }
}
