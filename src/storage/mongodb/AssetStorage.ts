...
export default class AssetStorage {
  ...
  public static async getAssetImage(tenant: Tenant, id: string): Promise<{ id: string; image: string }> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getAssetImage');
    // Check Tenant
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getAssetImage', uniqueTimerID, assetImageMDB);
    return {
      id: id,
  ...
  public static async saveAsset(tenant: Tenant, assetToSave: Asset, saveImage = true): Promise<string> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveAsset');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Set
    const assetMDB: any = {
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveAsset', uniqueTimerID, assetMDB);
  ...
  public static async getAssets(tenant: Tenant,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getAssets');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    if (dbParams.onlyRecordCount) {
      ...
      await Logging.traceEnd(tenant, MODULE_NAME, 'getAssets', uniqueTimerID, assetsCountMDB);
      return {
    ...
    const assetsMDB = await global.database.getCollection<any>(tenant.id, 'assets')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getAssets', uniqueTimerID, assetsMDB);
    return {
  ...
  public static async getAssetsInError(tenant: Tenant,
      ...
      dbParams?: DbParams, projectFields?: string[]): Promise<DataResult<Asset>> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getAssetsInError');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Clone before updating the values
    dbParams = Utils.cloneObject(dbParams);
    ...
    const assetsMDB = await global.database.getCollection<any>(tenant.id, 'assets')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getAssetsInError', uniqueTimerID, assetsMDB);
    // Ok
    return {
  ...
  public static async deleteAsset(tenant: Tenant, id: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteAsset');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Delete the Asset
    ...
    await global.database.getCollection<any>(tenant.id, 'assetimages')
      .findOneAndDelete({ '_id': DatabaseUtils.convertToObjectID(id) });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteAsset', uniqueTimerID, { id });
  }

  private static async saveAssetImage(tenant: Tenant, assetID: string, assetImageToSave: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveAssetImage');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Modify
    await global.database.getCollection<any>(tenant.id, 'assetimages').findOneAndUpdate(
      ...
      { upsert: true });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveAssetImage', uniqueTimerID, assetImageToSave);
  }
