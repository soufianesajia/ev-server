...
export default class TransactionStorage {
  ...
  public static async deleteTransactions(tenant: Tenant, transactionsIDs: number[]): Promise<number> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'deleteTransaction');
    // Check
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'deleteTransaction', uniqueTimerID, { transactionsIDs });
    return result.deletedCount;
  }

  public static async saveTransaction(tenant: Tenant, transactionToSave: Transaction): Promise<number> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveTransaction');
    // Check
    DatabaseUtils.checkTenantObject(tenant);
    // ID not provided?
    ...
    await global.database.getCollection<any>(tenant.id, 'transactions').findOneAndReplace(
      ...
      { upsert: true });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveTransaction', uniqueTimerID, transactionMDB);
    // Return
  ...
  public static async saveTransactionOcpiData(tenant: Tenant, id: number,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveTransactionOcpiData');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Modify document
    await global.database.getCollection<Transaction>(tenant.id, 'transactions').findOneAndUpdate(
      ...
      { upsert: false });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveTransactionOcpiData', uniqueTimerID, ocpiData);
  }

  public static async saveTransactionOicpData(tenant: Tenant, id: number,
      oicpData: TransactionOicpData): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveTransactionOicpData');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Modify document
    await global.database.getCollection<Transaction>(tenant.id, 'transactions').findOneAndUpdate(
      ...
      { upsert: false });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveTransactionOicpData', uniqueTimerID, oicpData);
  }

  public static async saveTransactionBillingData(tenant: Tenant, id: number,
      billingData: TransactionBillingData): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveTransactionBillingData');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Modify document
    await global.database.getCollection<Transaction>(tenant.id, 'transactions').findOneAndUpdate(
      ...
      { upsert: false });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveTransactionBillingData', uniqueTimerID, billingData);
  }

  public static async saveTransactionRefundData(tenant: Tenant, id: number,
      refundData: TransactionRefundData): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'saveTransactionRefundData');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    // Modify document
    await global.database.getCollection<Transaction>(tenant.id, 'transactions').findOneAndUpdate(
      ...
      { upsert: false });
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'saveTransactionRefundData', uniqueTimerID, refundData);
  }

  public static async assignTransactionsToUser(tenant: Tenant, userID: string, tagID: string): Promise<void> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'assignTransactionsToUser');
    // Assign transactions
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'assignTransactionsToUser', uniqueTimerID);
  }

  public static async getUnassignedTransactionsCount(tenant: Tenant, tagID: string): Promise<number> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getUnassignedTransactionsCount');
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getUnassignedTransactionsCount', uniqueTimerID);
    return unassignedCount;
  }

  public static async getTransactionYears(tenant: Tenant): Promise<Date[]> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getTransactionYears');
    // Check
    DatabaseUtils.checkTenantObject(tenant);
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getTransactionYears', uniqueTimerID, firstTransactionsMDB);
  ...
  public static async getTransactions(tenant: Tenant,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getTransactions');
    // Check
    DatabaseUtils.checkTenantObject(tenant);
    ...
    if (dbParams.onlyRecordCount) {
      await Logging.traceEnd(tenant, MODULE_NAME, 'getTransactions', uniqueTimerID, transactionCountMDB);
      return {
    ...
    const transactionsMDB = await global.database.getCollection<Transaction>(tenant.id, 'transactions')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getTransactions', uniqueTimerID, transactionsMDB);
    return {
  ...
  public static async getRefundReports(tenant: Tenant,
    ...
    // Check if only the total count is requested
    if (dbParams.onlyRecordCount) {
      await Logging.traceEnd(tenant, MODULE_NAME, 'getRefundReports', uniqueTimerID, reportCountMDB);
      return {
    ...
    const reportsMDB = await global.database.getCollection<RefundReport>(tenant.id, 'transactions')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getRefundReports', uniqueTimerID, reportsMDB);
    return {
  ...
  static async getTransactionsInError(tenant: Tenant,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getTransactionsInError');
    // Check
    DatabaseUtils.checkTenantObject(tenant);
    // Clone before updating the values
    dbParams = Utils.cloneObject(dbParams);
    ...
    const transactionsMDB = await global.database.getCollection<TransactionInError>(tenant.id, 'transactions')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getTransactionsInError', uniqueTimerID, transactionsMDB);
    return {
  ...
  public static async getActiveTransaction(tenant: Tenant, chargeBoxID: string, connectorId: number): Promise<Transaction> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getActiveTransaction');
    // Check
    DatabaseUtils.checkTenantObject(tenant);
    const aggregation = [];
    // Filters
    ...
    const transactionsMDB = await global.database.getCollection<Transaction>(tenant.id, 'transactions')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getActiveTransaction', uniqueTimerID, transactionsMDB);
  ...
  public static async getLastTransactionFromChargingStation(tenant: Tenant, chargeBoxID: string, connectorId: number,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getLastTransaction');
    // Check
    DatabaseUtils.checkTenantObject(tenant);
    const aggregation = [];
    // Filters
    ...
    const transactionsMDB = await global.database.getCollection<Transaction>(tenant.id, 'transactions')
      ...
      .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getLastTransaction', uniqueTimerID, transactionsMDB);
    return transactionsMDB.length === 1 ? transactionsMDB[0] : null;
  }

  public static async findAvailableID(tenant: Tenant): Promise<number> {
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, '_findAvailableID');
    // Check
    DatabaseUtils.checkTenantObject(tenant);
    ...
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, '_findAvailableID', uniqueTimerID);
  }
  ...
  public static async getNotStartedTransactions(tenant: Tenant,
    ...
    // Debug
    const uniqueTimerID = Logging.traceStart(tenant, MODULE_NAME, 'getNotStartedTransactions');
    // Check Tenant
    DatabaseUtils.checkTenantObject(tenant);
    ...
    const notifySessionNotStartedMDB: NotifySessionNotStarted[] =
      await global.database.getCollection<NotifySessionNotStarted>(tenant.id, 'authorizes')
        ...
        .toArray();
    // Debug
    await Logging.traceEnd(tenant, MODULE_NAME, 'getNotStartedTransactions', uniqueTimerID, notifySessionNotStartedMDB);
    return {
